...
download_start = pd.to_datetime(START) - timedelta(days=2 * max(34, LONG_WINDOW))

# Se descargan los datos de precios
price_data = download_price_data(TICKERS, start_date=download_start, end_date=END)
...
combined_data = combined_data[combined_data.index >= pd.to_datetime(START)]
